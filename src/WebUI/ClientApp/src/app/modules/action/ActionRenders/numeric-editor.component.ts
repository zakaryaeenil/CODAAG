...
export class NumericEditorComponent implements ICellEditorAngularComp, AfterViewInit {
  ...
  onKeyDown(event: any): void {
    if (NumericEditorComponent.isLeftOrRight(event) || NumericEditorComponent.deleteOrBackspace(event)) {
      event.stopPropagation();
      return;
    }

    if (
      !NumericEditorComponent.finishedEditingPressed(event) &&
      !NumericEditorComponent.isKeyPressedNumeric(event)
    ) {
