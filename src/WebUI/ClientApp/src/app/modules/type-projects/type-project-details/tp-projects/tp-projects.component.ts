...
@Component({
  selector: 'app-tp-projects',
  templateUrl: './tp-projects.component.html',
  styleUrls: ['./tp-projects.component.scss']
})
export class TpProjectsComponent  {
  @Input()  tp_pTable : TypeProject | undefined;
  ...
  constructor(private liststp: TypeProjectsClient, private router: ActivatedRoute) {
    liststp.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.typeProjectDto?.projects
      },
  ...
  onGridReady(params: GridReadyEvent) {
    this.rowData$ = this.vm.typeProjectDto?.projects!
  ...
  export() {
    ...
    const title = 'Projects for TypeProject_' + this.vm.typeProjectDto?.title;
    let d = new Date();
