...
export class CoProjectsComponent  {
  ...
  constructor(private listsStatut: ContratObjectifsClient, private router: ActivatedRoute) {
    listsStatut.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.contratObjectifDto?.projects
      },
  ...
  onGridReady(params: GridReadyEvent) {
    this.rowData$ = this.vm.contratObjectifDto?.projects!
  ...
  export() {
    ...
    const title = 'Projects for Contrat_' + this.vm.contratObjectifDto?.title;
    let d = new Date();
