...
import {Structure, StructureByIdVm, StructuresClient} from "../../../../web-api-client";
...
export class ScStructureComponent {
  @Input()  str_scTable : Structure | undefined;
  ...
  constructor(private listsStructures : StructuresClient, private router : ActivatedRoute){
    listsStructures.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.structureDto?.structureChildren
  ...
  onGridReady(params: GridReadyEvent) {
    ...
    this.rowData$ = this.vm.structureDto?.structureChildren!
  ...
  export() {
    ...
    const title = 'Structures Child for_'+this.str_scTable?.title ;
    let d = new Date();
