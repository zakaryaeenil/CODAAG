...
import {Gestionnaire, Project, Structure, StructureByIdVm, StructuresClient} from "../../../../web-api-client";
...
export class PStructureComponent {
  @Input()  str_pTable : Structure | undefined;
...
// DefaultColDef sets props common to all Columns
  ...
  constructor(private listsStructures : StructuresClient, private router : ActivatedRoute){
    listsStructures.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.structureDto?.projects
  ...
  onGridReady(params: GridReadyEvent) {
    ...
    this.rowData$ = this.vm.structureDto?.projects!
  ...
  export() {
    ...
    const title = 'Projects for Structure_' + this.vm.structureDto?.title;
    let d = new Date();
