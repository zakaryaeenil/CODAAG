...
import {ContratObjectif, Statut, StatutByIdVm, StatutsClient} from "../../../../web-api-client";
...
@Component({
  selector: 'app-s-contrats-view',
  templateUrl: './s-contrats-view.component.html',
  styleUrls: ['./s-contrats-view.component.scss']
})
export class SContratsViewComponent {
  @Input()  s_cTable : Statut | undefined;
  ...
  constructor(private listsStatut : StatutsClient, private router : ActivatedRoute){
    listsStatut.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.statutDto?.contratObjectifs
      },
  ...
  onGridReady(params: GridReadyEvent) {
    this.rowData$ = this.vm.statutDto?.contratObjectifs!
  ...
  export() {
    ...
    const title = 'Contrats for Statut_' + this.s_cTable?.title;
    let d = new Date();
    let date = d.getDate() + '-' + d.getMonth() + '-' + d.getFullYear();

    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Contrats for Statut_' + this.s_cTable?.title);
