...
import {ActionP, ContratObjectif, Statut, StatutByIdVm, StatutsClient} from "../../../../web-api-client";
...
@Component({
  selector: 'app-s-actions-view',
  templateUrl: './s-actions-view.component.html',
  styleUrls: ['./s-actions-view.component.scss']
})
export class SActionsViewComponent {

  @Input()  s_aTable : Statut | undefined;
  ...
  constructor(private listsStatut : StatutsClient, private router : ActivatedRoute){
    listsStatut.get2(router.snapshot.params['id']).subscribe(
      result => {
        this.vm = result;
        this.rowData$ = result.statutDto?.actionPs
      },
  ...
  onGridReady(params: GridReadyEvent) {
    this.rowData$ = this.vm.statutDto?.actionPs!
  ...
  export() {
    ...
    const title = 'Actions for Statut_'+this.vm.statutDto?.title;
    let d = new Date();
    let date = d.getDate() + '-' + d.getMonth() + '-' + d.getFullYear();

    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Actions for Statut_'+this.vm.statutDto?.title);
